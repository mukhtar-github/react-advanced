...
So the good news here is that, if we're just working with the count state value, we're not going to trigger a re-render. So now we'll only create addToCart function when we updte the cart value.
